...
parameters{
  ...
  matrix[S,n_occasions-1] phi_0_raw; // intercept for survival model
...
transformed parameters{
  ...
  matrix[S,n_occasions-1] phi_0; // intercept for survival model
  ...
  for(s in 1:S){
    ...
    phi_0[s,i] = mu_0 + sig_0 * phi_0_raw[s,i];
  }
  }
...
for(s in 1:S){
  ...
  for(i in 1:n_occasions - 1){
   phi[s, i] = inv_logit(phi_0[s,i]); // survival is a function of predictor vars
  }
