...
parameters{
  real<lower = 0, upper = 1> real_p[S]; // detection probability for all years
  vector[S] phi_0_raw; // intercept for survival model
  vector[S] phi_temp_raw; // effect of temp on survival
  real mu_0; // mean for phi_0 distribution
  real<lower = 0> sig_0; // sd for phi_0 distribution
  real mu_temp; // mean for phi_temp distribution
  real<lower = 0> sig_temp; // sd for phi_temp distribution
}
...
transformed parameters{
  ...
  vector[S] phi_temp; // effect of prec on survival
  ...
  phi_temp = mu_temp + sig_temp * phi_temp_raw;
...
for(s in 1:S){
  ...
  for(i in observed){
    p[s, i] = real_p[s]; // fill in sampled p with a real estimate
  }
  
  for(i in 1:n_occasions - 1){
   phi[s, i] = inv_logit(phi_0[s]+temp[i]*phi_temp[s]); // survival is a function of predictor vars
  }
